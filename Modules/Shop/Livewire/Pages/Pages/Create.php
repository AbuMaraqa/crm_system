...
class Create extends Component implements HasForms
{
    ...
    public function saving(): void
    {
        ...
        QueryContainer::make()
            ->wrap(function () {
                ...
                $brand = Page::create($formattedData);

                if (isset($data['image']) && $data['image'] instanceof TemporaryUploadedFile) {
                    $brand->addMedia($data['image']->getRealPath())->toMediaCollection('about_us');
                }
    ...
    {
        $this->redirectRoute('dashboard.shop.pages');
    }
    ...
    {
        return view('shop::livewire.pages.page.create')
            ->layout(AppLayouts::class, [
                'pageTitle'   => __('Create Page'),
                'breadcrumbs' => [
                    route('dashboard.home')               => __('Home'),
                    route('dashboard.shop.pages')        => __('Pages'),
                    route('dashboard.shop.pages.create') => __('Create Page'),
                ],
            ]);
    }
}
